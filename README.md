...
that API use `getUserMedia` for camera and microphone broadcasts. Mobile users
can choose camera plus microphone, camera only, or microphone only, and can
switch between the front and rear cameras while live. The extension is
optional — install it only for one-click tab capture or background
broadcasting.
...
| **In-browser broadcaster**       | `getDisplayMedia` on desktop or `getUserMedia` on mobile → PeerJS host; screen, window, tab, camera, or microphone. |
